...
**Storage — this is central:** everything lives in `IndexedDB` (db `rpm-blobs`, store `blobs`), accessed via the `DB` helper (`put`/`get`/`del`, all promise-based). Nothing is stored in `localStorage` anymore.
- Project configs (the whole `state` array, plus `selectedId`) are one record under key `meta:state`, written as `{projects, selectedId}`. Auto-backup snapshots (see below) are under `meta:snapshots`.
- Why not `localStorage`: it was tried first (config in `localStorage`, blobs in `IndexedDB`), but `localStorage` is a small quota (~5-10MB) shared across **every** page hosted at the same origin — on GitHub Pages user sites (`<user>.github.io`), that's shared with any *other* project of yours at that domain, so an unrelated page can silently exhaust the quota and break saves here with no way for this app to free space. `IndexedDB`'s quota is far larger (typically a percentage of free disk, easily GBs) and not practically contended the same way. `loadState()` does a one-time migration from the old `rpm.projects.v1`/`rpm.selected.v1`/`rpm.snapshots.v1` `localStorage` keys if `meta:state` doesn't exist yet, then clears those keys.
- Binary blobs use the same store, keyed by convention: uploaded source ZIPs by the source's `id`; project icon under `icon:<projectId>`; per-item-texture images under `cit:<projectId>:<ruleId>`; per-block-texture images under `blk:<projectId>:<ruleId>:<face>` (`face` is `all` for uniform mode or one of `up/down/north/south/east/west`); per-sound audio under `snd:<projectId>:<ruleId>`.
- `save()` is fire-and-forget from every call site (not awaited) — it kicks off a `DB.put` and handles its own errors internally (`alert()` on failure), matching the old synchronous-`localStorage` calling convention so none of the ~20 call sites needed to become `async`.
